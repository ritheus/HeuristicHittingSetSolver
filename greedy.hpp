...
namespace Greedy {
	...
	void setCount(pq::updatable_priority_queue<Node, uint32_t>&, const std::vector<std::vector<Node>>&, std::vector<std::vector<uint32_t>>&);
	void updateCount(pq::updatable_priority_queue<Node, uint32_t>&, const std::vector<std::vector<Node>>&);
	std::vector<std::vector<Node>> removeEdgesContainingNode(Node, std::vector<std::vector<Node>>&, std::vector<std::vector<uint32_t>>&, std::unordered_set<uint32_t>&);
}